...
def delete_information():
    ...
    if not receipt_number or not item_number:
        messagebox.showwarning("Warning", "Please enter both a receipt number and item number.")
        return
    ...
    try:
        ...
        with open("Reciepts.txt", "w") as file:
            for record in records:
                if (f"ID:{receipt_number}" in record) and (f"Item No:{item_number}" in record):
                    found = True
                    continue
                file.write(record)
        
        if found:
            messagebox.showinfo("Success", f"Record with Receipt Number {receipt_number} and Item Number {item_number} deleted successfully.")
        else:
            messagebox.showwarning("Warning", f"Record with Receipt Number {receipt_number} and Item Number {item_number} not found.")
...
def updated_equipment_hiring():
    ...
    tk.Button(root7, text="Place Order", command=save_purchase, **btn_config, bg="#90EE90", fg="white").pack(pady=10)
    tk.Button(root7, text="Back", command=lambda: [root7.destroy(), main_button_window()], bg="red", fg="white", font=("Times New Roman", 12)).pack(pady=10)
